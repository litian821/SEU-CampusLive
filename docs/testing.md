...
1. 按 `docs/streaming.md` 配置一场至少包含两个机位的比赛，并重建 API。
2. 两台手机分别使用不同流名开始推流；不得复制同一个 RTMP 完整地址。
3. 请求 `GET /api/matches/<match-id>`，确认两个机位都为 `live`。
4. 打开 `/live/<match-id>`，确认页面显示正确的机位总数和在线数。
5. 在两个机位之间来回切换，确认画面、机位名称和声音均对应当前选择；浏览器网络面板中只应持续请求当前一路的 HLS 分片。
6. 停止其中一台手机，等待状态刷新，确认该机位变为未开播且另一路继续播放。
7. 再配置第二场比赛并推送它的流，确认直播目录按比赛显示两张卡片，进入后不会串流。
