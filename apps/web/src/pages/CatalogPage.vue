...
import { getLiveStreams, getVodItems } from '../api'
...
const { data, error, loading, retry } = useCatalog<Card[]>(async signal => props.kind === 'live'
  ? (await getLiveStreams(signal)).map(liveCard) : (await getVodItems(signal)).map(vodCard), props.kind === 'live' ? 10000 : 0)
...
<template><div class="container">
  ...
  <p class="catalog-count">{{ error ? '目录暂不可用' : `共 ${filtered.length} ${kind === 'live' ? '个频道' : '个视频'}` }} <span v-if="kind === 'vod'">按标题排列</span></p>
